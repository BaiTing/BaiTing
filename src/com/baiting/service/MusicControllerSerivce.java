...
public class MusicControllerSerivce extends MusicService implements Runnable{
	...
	private synchronized void init() {
		...
		try {
			long fileSize = 0l;
			if(isNetPlay) {
				if(SettingConfig.getInstance().isUseProxy()){ //需要代理上网
		    	    ...
		    	    if (statusCode == HttpStatus.SC_OK){
		    	    	...
		    	    	if(response.getEntity().isStreaming()){
		    	    		...
		    	    		aff = AudioSystem.getAudioFileFormat(in) ;
		    	    	}
		    	    }
		    	}else{ //无需代理上网
	...
	public synchronized void play() {
		if(status == PAUSE_STATUS) {
			...
			if(null != playSong) {
			log.info("正在播放[ "+playSong.getName()+" ]");
			}
			setStatus(OPENED);
		}
	}
...
//				   boolean flag =  pb.open(playSong.getUrl(), playSong.getName()) ;
//				   if(flag)
					   setStatus(PLAY_STATUS);
				}
				log.info("---正在播放---"+playSong.getName());
